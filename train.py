...
from utils.utils import clip_gradient, AvgMeter, poly_lr, adjust_lr
import torch.nn.functional as F
import numpy as np
import torch.nn as nn


file = open("log/GBNet.txt", "a")
...
def train(train_loader, model, optimizer, epoch):
    ...
    for i, pack in enumerate(train_loader, start=1):
        ...
        loss = 3*loss3 + 2*loss2 + loss1 + 6*losse
        # ---- backward ----
        loss.backward()
        ...
        if i % 60 == 0 or i == total_step:
            print('{} Epoch [{:03d}/{:03d}], Step [{:04d}/{:04d}], '
                  '[lateral-3: {:.4f}], [lateral-2: {:.4f}], [lateral-1: {:.4f}], [lateral-1: {:,.4f}]'.
    ...
    if (((epoch + 1) % 5 == 0 or (epoch + 1) == opt.epoch) and epoch > 14):
        torch.save(model.state_dict(), save_path + 'GBNet-%d.pth' % epoch)
        print('[Saving Snapshot:]', save_path + 'GBNet-%d.pth' % epoch)
        file.write('[Saving Snapshot:]' + save_path + 'GBNet-%d.pth' % epoch + '\n')
