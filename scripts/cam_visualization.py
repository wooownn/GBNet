"""
Code from Go Closer to See Better: Camouflaged Object Detection via Object Area Amplification and Figure-Ground Conversion
"""
...
from net.gbnet import Net                  #your net
...
def main(exp_name, net, pretrained_model_path, modules_for_plot, scale ): 
    ...
    with torch.no_grad():
        for name, root in to_test.items():
            ...
            img_list = [os.path.splitext(f)[0] for f in os.listdir(image_path) if f.endswith('png') or f.endswith("jpg")]
            img_list.sort()
...
def predict_model(modelname,pretrained_model_path,net): ##Your modelname , model_path , net name
    # net.load_state_dict(torch.load(pretrained_model_path))
    # print(net.bem.block[5])
    # exit()
    modules_for_plot = OrderedDict([
        ('net.bem.block[5]', net.bem.block[5]),
        
        
    ])  ###The network layer you need to output

    main(modelname,net,pretrained_model_path,modules_for_plot,scale=704)

if __name__ == '__main__': 
    predict_model('NET','./checkpoints/GBNet/GBNet-29.pth', Net().to("cuda:0"))       ###checkpoints  pth