...
def pvt_v1(pretrained=True):
    model = pvt_large()
    if pretrained:
        print('pvt_large and pretrained=True')
        # model_state = torch.load('../lib/models/res2net50_v1b_26w_4s-3cf99910.pth',map_location='cpu')
        model_state = torch.load('/home/ubuntu/wxd/cod/code/BGNet/models/pvt_large.pth')
        # 加载模型参数
        new_model_state = OrderedDict()
        for k,v in model_state.items():
            if k != 'cls_token' and k != 'norm.weight' and k != 'norm.bias' and k != 'head.weight' and k != 'head.bias':
                new_model_state[k] = v
...
class Attention(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        if self.sr_ratio > 1:
            ...
            x_ = self.sr(x_).reshape(B, C, -1).permute(0, 2, 1) # (B,C,H,W)->(B,C,N)->(B,N,C)
            x_ = self.norm(x_)
            kv = self.kv(x_).reshape(B, -1, 2, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
            #(B,N,C) -->(B,N,2*C) -->(B,N,2,heads,C//heads) -->(2,B,heads,N,C//heads)
        else:
            kv = self.kv(x).reshape(B, -1, 2, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        k, v = kv[0], kv[1]
        # (B, heads, N, C//heads) * (B, heads, C//heads, N)
        attn = (q @ k.transpose(-2, -1)) * self.scale
        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn) # (B, heads, N, N)
        # (B, heads, N, N) * (B, heads, N, C//heads)--> (B,heads,N,C//heads)
...
class PatchEmbed(nn.Module):
    ...
    """
    def __init__(self, img_size=352, patch_size=16, in_chans=3, embed_dim=768):
        super().__init__()
...
class PyramidVisionTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000, embed_dims=[64, 128, 320, 512],
                 ...
                 depths=[2, 2, 2, 2], sr_ratios=[8, 4, 2, 1], num_stages=4, F4=False):
        super().__init__()
        # self.num_classes = num_classes
        self.depths = depths
        self.F4 = F4
        self.num_stages = num_stages # 四阶段
        ...
        for i in range(num_stages): # i=0,1,2,3
            ...
            # num_patches为经过patch embedding缩小后的H\W
            num_patches = patch_embed.num_patches if i != num_stages - 1 else patch_embed.num_patches + 1
            # 可训练参数 位置编码：position embedding
            pos_embed = nn.Parameter(torch.zeros(1, num_patches, embed_dims[i]))
            pos_drop = nn.Dropout(p=drop_rate)
            # 二、Transformer Encoder * depth[i]---------------------------------------------------------
